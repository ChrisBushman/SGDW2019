...
var cursors;
// State of if the game has "started"
var gameStart = false;
...
function update() {
  ...
  // Before game starts, check for players to join
  if (gameStart === false) {
    initPlayers();
  }
...
// If a player presses 'A' on an available gamepad, then a player object is initalized into the game
function initPlayers() {
  if (sqPlayer1 === undefined && (gamepad1.connected &&
     gamepad1.justPressed(Phaser.Gamepad.XBOX360_A))) {
    sqPlayer1 = game.add.sprite(100, 100, 'player');
  }
  
  if (sqPlayer2 === undefined && (gamepad2.connected &&
     gamepad2.justPressed(Phaser.Gamepad.XBOX360_A))) {
    // We are using 'Sprites' instead of 'Images' to allow tinting to differentiate players
    sqPlayer2 = game.add.sprite(500, 100, 'player');
    sqPlayer2.tint = 0x777777;
  }
  
  if (sqPlayer3 === undefined && (gamepad3.connected &&
     gamepad3.justPressed(Phaser.Gamepad.XBOX360_A))) {
    sqPlayer3 = game.add.sprite(100, 500, 'player');
    sqPlayer3.tint = 0xAAAAAA;
  }
  
  if (sqPlayer4 === undefined && (gamepad4.connected &&
